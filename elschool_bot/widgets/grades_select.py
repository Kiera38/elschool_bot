...
async def on_start_lesson_date(event, button, manager: DialogManager):
    ...
    await manager.start(
        DateSelectorStates.SELECT_VARIANT, manager.dialog_data.get("lesson_dates")
    )
...
async def on_start_date(event, button, manager: DialogManager):
    ...
    await manager.start(
        DateSelectorStates.SELECT_VARIANT, manager.dialog_data.get("dates")
    )
...
def create(lessons_state, lesson_date_state):
    ...
    select_date = Button(Format("дата проставления"), "date", on_start_date)

    if lesson_date_state:
        center = (
            select_lessons,
            Row(
                Button(Format("дата урока"), "date_lesson", on_start_lesson_date),
                select_date,
            ),
        )
    else:
