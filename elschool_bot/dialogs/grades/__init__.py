...
async def handle_register_error(manager, repo, error):
    ...
    if login is None and password is None:
        await input_data.start(
            ["логин", "пароль"],
            (f"{text} У меня не сохранены твои данные", ""),
            manager,
            check_get_grades=False,
        )
    elif login is None:
        manager.dialog_data.update(password=password)
        await input_data.start(
            ["логин"],
            (f"{text} У меня не сохранён твой пароль", ""),
            manager,
            check_get_grades=False,
            value=password,
        )
        ...
        await input_data.start(
            ["пароль"],
            (f"{text} У меня не сохранён твой логин", ""),
            manager,
            check_get_grades=False,
            value=login,
        )
        ...
        await status.update(
            manager, f"{text}. Сейчас обновлю у себя", show_mode=ShowMode.EDIT
        )
        try:
...
async def update_token(login, password, jwtoken, manager, save_data):
    repo = manager.middleware_data["repo"]
    await status.update(
        manager, "обновление токена: попытка регистрации", show_mode=ShowMode.EDIT
    )
...
async def get_grades_after_error(manager, repo):
    await status.update(
        manager, "данные введены правильно, теперь попробую получить оценки"
    )
    try:
        grades = await repo.get_grades(manager.event.from_user.id)
    except RegisterError as e:
...
async def show_select(grades, manager: DialogManager):
    status.set(
        manager,
        "оценки получил, теперь можешь выбрать",
        checked_lessons="",
        checked_date_lesson="",
        checked_date="",
        grades=grades,
...
async def on_show(query, button, manager: DialogManager):
    ...
    marks_selected = {
        int(mark) for mark in manager.find("marks_selector").get_checked()
    }
    ...
    if grades_select.is_statistics_checked(manager):
        ...
        value_filters = (
            ...
            filter_lesson_date(dates, "date"),
            filter_marks(marks_selected),
        )
