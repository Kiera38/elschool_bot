...
async def select_schedule(query, manager: DialogManager, item, grades):
    ...
    (
        ...
        marks,
        show_without_marks,
    ) = await repo.get_schedule(query.from_user.id, item)
...
async def on_process_result(start_data, result, manager: DialogManager):
    ...
    if manager.dialog_data.get("schedule_id"):
        grades = await process_result(start_data, result, manager)
        if grades:
            await select_schedule(
                manager.event, manager, manager.dialog_data["schedule_id"], grades
            )
...
async def on_input_custom_time(
    message, text_input, manager: DialogManager, input_message
):
...
dialog = Dialog(
    Window(
        ...
        Row(Button(Const("новая отправка"), "new", on_new), Cancel(Const("назад"))),
        Column(
            Select(
                Format("{item[1]}"),
                "select",
    ...
    select_lessons.create(
        SchedulerStates.SELECT_LESSONS, SchedulerStates.SCHEDULE_GRADES_SELECT
    ),
    Window(
