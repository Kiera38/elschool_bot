...
async def on_start(data, manager: DialogManager):
    repo = manager.middleware_data["repo"]
    notify_change_schedule = await repo.get_user_notify_change_schedule(
...
dialog = Dialog(
    Window(
        ...
        Button(
            Const("автоматическая отправка расписания"),
            "autosend_schedule_btn",
            on_autosend_schedule,
        ),
        Checkbox(
    ...
    Window(
        ...
        Radio(
            ...
            when=lambda data, radio, manager: manager.find(
                "autosend_schedule"
            ).is_checked(),
        ),
        Checkbox(
        ...
        Group(
            Radio(
                ...
                when=lambda data, radio, manager: (
                    manager.find("loop").is_checked()
                    and manager.find("autosend_schedule").is_checked()
                ),
            ),
            width=2,
        ),
