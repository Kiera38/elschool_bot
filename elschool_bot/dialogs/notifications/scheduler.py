...
from elschool_bot.dialogs.grades import (
    start_get_grades,
    process_result,
    show_default,
    filter_marks,
    filter_selected,
    filter_without_marks,
    show_statistics,
)
...
class Scheduler:
    ...
    async def show_id(self, manager: BaseDialogManager, id, delay):
        ...
        await manager.start(
            SchedulerShowStates.STATUS,
            {"notifications": self, "id": id},
            StartMode.NEW_STACK,
        )
...
async def select_grades(grades, manager: DialogManager):
    ...
    (
        ...
        marks,
        show_without_marks,
    ) = await repo.get_schedule(user_id, id)
    ...
    if show_mode == 0:
        ...
        await show_statistics(
            grades, manager, marks_selected, filters, value_filters, False
        )
