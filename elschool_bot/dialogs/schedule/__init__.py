...
async def start_schedule(manager: BaseDialogManager, schedule, day):
    await manager.start(
        ScheduleStates.STATUS,
        {"type": "schedule", "schedule": schedule, "day": day},
        mode=StartMode.NEW_STACK,
    )


async def on_select_day(
    event: ChatEvent, widget: ManagedCalendar, manager: DialogManager, date
):
...
def as_datetime(time):
    ...
    return (datetime.datetime.utcnow() + datetime.timedelta(hours=5)).replace(
        ...
        if end_time < now < start_time:
            ...
            for lesson in lessons:
                ...
                if now > end_time:
                    ...
                    current = (
                        "я вроде все ситуации учитываю, но эту чего-то не знаю. "
...
async def on_process_result(start_data, result, manager: DialogManager):
    if await grades.process_results_without_grades(start_data, result, manager):
        ...
        if manager.dialog_data.get("marks"):
            marks = await get_marks_after_error(
                manager, repo, manager.event.from_user.id
            )
            if marks:
...
async def on_edit(event, button, manager: DialogManager):
    ...
    await edit.start(
        manager.dialog_data["schedule"].copy(), manager.dialog_data["day"], manager
    )
...
async def on_select_homework_lesson(event, select, manager: DialogManager, item):
    manager.dialog_data["selected_lesson"] = manager.dialog_data["schedule"][int(item)][
        "name"
    ]
...
async def on_input_homework(event, text_input, manager: DialogManager, text):
    ...
    if day:
        ...
        manager.dialog_data["status"] = (
            f"не найден день, когда урок {lesson} будет в следующий раз. "
...
dialog = Dialog(
    Window(
        ...
        SwitchTo(
            ...
            "default_edit",
            ScheduleStates.SELECT_DEFAULT_EDIT_DAY,
        ),
    ...
    Window(
        ...
        Row(
            Checkbox(
                Const("✓ оценки"), Const("оценки"), "marks", on_state_changed=on_marks
            ),
