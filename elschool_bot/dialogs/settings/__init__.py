...
async def on_version(query, button, manager: DialogManager):
    await status.update(manager, '''моя версия: 3.3.0.dev8

Список изменений:
в 3.3.0:
просмотр расписания, просмотр расписания звонков, кривой код, можно указывать изменения в расписании (можно считать эксперементальной функцией).
